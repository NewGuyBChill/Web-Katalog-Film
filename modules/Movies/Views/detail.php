...
.cast-scroll::-webkit-scrollbar-thumb { background: var(--accent); border-radius: 4px; }
</style>
<?php
...
<main style="padding-top: 80px; min-height: 80vh;">
    ...
    <!-- Bagian Artis dan Pemeran -->
    <div class="container" style="max-width: 1100px; margin: 0 auto; padding-top: 3rem;">
        <h3 style="font-size: 1.5rem; margin-bottom: 1.5rem; color: var(--text-main); font-weight: 600; border-left: 4px solid var(--accent); padding-left: 10px;"><?= translateText('cast_and_crew') ?></h3>
        <div class="cast-scroll" style="display: flex; gap: 1rem; overflow-x: auto; padding-bottom: 1rem;">
            <?php if (!empty($movie['credits']['cast'])): ?>
                <?php $castList = array_slice($movie['credits']['cast'], 0, 10); ?>
                <?php foreach ($castList as $actor): ?>
                    <?php $actorImg = !empty($actor['profile_path']) ? "https://image.tmdb.org/t/p/w200" . $actor['profile_path'] : "data:image/svg+xml;charset=UTF-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22200%22%20height%3D%22300%22%20viewBox%3D%220%200%20200%20300%22%3E%3Crect%20width%3D%22200%22%20height%3D%22300%22%20fill%3D%22%231a1a1a%22%2F%3E%3Ctext%20x%3D%2250%25%22%20y%3D%2250%25%22%20font-family%3D%22sans-serif%22%20font-size%3D%2216%22%20fill%3D%22%23555555%22%20text-anchor%3D%22middle%22%20dominant-baseline%3D%22middle%22%3ENo%20Image%3C%2Ftext%3E%3C%2Fsvg%3E"; ?>
                    <div style="min-width: 140px; background: rgba(255,255,255,0.05); border-radius: 12px; overflow: hidden; text-align: center; border: 1px solid rgba(255,255,255,0.1);">
                        <img src="<?= $actorImg ?>" alt="<?= htmlspecialchars($actor['name']) ?>" style="width: 100%; height: 210px; object-fit: cover;">
                        <div style="padding: 10px;">
                            <div style="font-weight: 600; font-size: 0.9rem; margin-bottom: 4px; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="<?= htmlspecialchars($actor['name']) ?>"><?= htmlspecialchars($actor['name']) ?></div>
                            <div style="font-size: 0.8rem; color: var(--text-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="<?= htmlspecialchars($actor['character']) ?>"><?= htmlspecialchars($actor['character']) ?></div>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <p style="color: var(--text-muted);">Tidak ada informasi artis/pemeran.</p>
            <?php endif; ?>
        </div>
    </div>
